...
class CoindbCommand extends CConsoleCommand
{
	...
	*/
	...
	{
		$url = 'https://www.bit-z.com/images/coins/coin_'; // https://www.bit-z.com/images/coins/coin_btc.png
		$nbUpdated = 0;
		$sql = "SELECT DISTINCT coins.id FROM coins INNER JOIN markets M ON M.coinid = coins.id WHERE M.name='bitz' AND IFNULL(coins.image,'') = ''";
		$coins = dbolist($sql);
		if (empty($coins))
			return 0;
		echo "bter: try to download new icons...\n";
